...
def group_expr(terms, group_size=32):
    """Wraps long unrolled additions in parentheses to prevent SQLite AST depth OOM"""
    if not terms: return "0.0"
    groups = ["(" + " + ".join(terms[i:i+group_size]) + ")" for i in range(0, len(terms), group_size)]
    return " + ".join(groups)
...
def build_optimized_sql_db():
    ...
    d_cols = ", ".join(f"d{i} REAL" for i in range(HIDDEN_DIM))
    d_qs = ", ".join("?" for _ in range(HIDDEN_DIM))
    
    tables = [
        f"w_emb (token INT PRIMARY KEY, {d_cols})",
        f"w_norm (layer INT PRIMARY KEY, {d_cols})",
        f"w_q (layer INT, head INT, h_dim INT, {d_cols})",
        f"w_k (layer INT, head INT, h_dim INT, {d_cols})",
        f"w_v (layer INT, head INT, h_dim INT, {d_cols})",
        f"w_o (layer INT, in_idx INT, {d_cols})",
        f"w_ffn_norm (layer INT PRIMARY KEY, {d_cols})",
        f"w_ffn_gate (layer INT, out_dim INT, {d_cols})",
        f"w_ffn_up (layer INT, out_dim INT, {d_cols})",
        f"w_ffn_down (layer INT, in_idx INT, {d_cols})",
        f"w_out_norm (id INT PRIMARY KEY, {d_cols})",
        f"w_out (token INT PRIMARY KEY, {d_cols})",
    ...
    # Generate the waterfall state tables to bypass single-trigger memory limits
    tables.extend([f"step_{i} (pos INT, token INT)" for i in range(31)])
    ...
    c.execute("INSERT INTO hyperparams VALUES (1, 0.8, 0.9, 40, 0.1, 1.2, 8192)")
    ...
    c.executemany(f"INSERT INTO w_emb VALUES (?, {d_qs})", [[t] + row for t, row in enumerate(to_matrix(sd["model.embed_tokens.weight"]))])
    c.executemany(f"INSERT INTO w_out VALUES (?, {d_qs})", [[t] + row for t, row in enumerate(to_matrix(sd["lm_head.weight"]))])
    c.execute(f"INSERT INTO w_out_norm VALUES (0, {d_qs})", to_list(sd["model.norm.weight"]))
    ...
    for i in range(30):
        print(f"  -> Processing Layer {i}/29...")
        prefix = f"model.layers.{i}."
        c.execute(f"INSERT INTO w_norm VALUES (?, {d_qs})", [i] + to_list(sd[prefix+"input_layernorm.weight"]))
        c.execute(f"INSERT INTO w_ffn_norm VALUES (?, {d_qs})", [i] + to_list(sd[prefix+"post_attention_layernorm.weight"]))
        c.executemany(f"INSERT INTO w_q VALUES (?, ?, ?, {d_qs})", [[i, h // 64, h % 64] + r for h, r in enumerate(to_matrix(sd[prefix+"self_attn.q_proj.weight"]))])
        c.executemany(f"INSERT INTO w_k VALUES (?, ?, ?, {d_qs})", [[i, h // 64, h % 64] + r for h, r in enumerate(to_matrix(sd[prefix+"self_attn.k_proj.weight"]))])
        c.executemany(f"INSERT INTO w_v VALUES (?, ?, ?, {d_qs})", [[i, h // 64, h % 64] + r for h, r in enumerate(to_matrix(sd[prefix+"self_attn.v_proj.weight"]))])
        c.executemany(f"INSERT INTO w_ffn_gate VALUES (?, ?, {d_qs})", [[i, idx] + r for idx, r in enumerate(to_matrix(sd[prefix+"mlp.gate_proj.weight"]))])
        c.executemany(f"INSERT INTO w_ffn_up VALUES (?, ?, {d_qs})", [[i, idx] + r for idx, r in enumerate(to_matrix(sd[prefix+"mlp.up_proj.weight"]))])
        c.executemany(f"INSERT INTO w_o VALUES (?, ?, {d_qs})", [[i, idx] + list(r) for idx, r in enumerate(list(zip(*to_matrix(sd[prefix+"self_attn.o_proj.weight"]))))])
        c.executemany(f"INSERT INTO w_ffn_down VALUES (?, ?, {d_qs})", [[i, idx] + list(r) for idx, r in enumerate(list(zip(*to_matrix(sd[prefix+"mlp.down_proj.weight"]))))])
    ...
    print("Forging the High-Speed Waterfall Triggers...")
    d_select = ", ".join(f"d{i}" for i in range(HIDDEN_DIM))
    
    # GROUPED EXPRESSIONS (The AST Memory Fix)
    dot_sq = group_expr([f"val.d{i}*val.d{i}" for i in range(HIDDEN_DIM)])
    rms_calc = f"1.0 / SQRT(({dot_sq}) / {float(HIDDEN_DIM)} + 1e-5)"
    dot_prod = group_expr([f"w.d{i} * x.d{i}" for i in range(HIDDEN_DIM)])
    
    sql = [f"""
    CREATE TRIGGER IF NOT EXISTS llm_forward_pass
    AFTER INSERT ON generation_loop
    BEGIN
        DELETE FROM t_x; INSERT INTO t_x SELECT {d_select} FROM w_emb WHERE token = NEW.token;
        -- Start Waterfall
        DELETE FROM step_0; INSERT INTO step_0 VALUES (NEW.pos, NEW.token);
    END;
    """]

    for L in range(30):
        norm_calc = ", ".join(f"val.d{i} * stats.rms * wn.d{i} as d{i}" for i in range(HIDDEN_DIM))
        o_sum = ", ".join(f"COALESCE(SUM(a.val * w.d{i}), 0.0) as d{i}" for i in range(HIDDEN_DIM))
        d_sum = ", ".join(f"COALESCE(SUM(s.val * w.d{i}), 0.0) as d{i}" for i in range(HIDDEN_DIM))
        o_update = ", ".join(f"d{i} = t_x.d{i} + COALESCE(delta.d{i}, 0.0)" for i in range(HIDDEN_DIM))
        
        sql.append(f"""
        CREATE TRIGGER IF NOT EXISTS llm_layer_{L}
        AFTER INSERT ON step_{L}
        BEGIN
            ...
            -- Chain to next Layer
            DELETE FROM step_{L+1}; INSERT INTO step_{L+1} VALUES (NEW.pos, NEW.token);
        END;
        """)

    final_norm_calc = ", ".join(f"val.d{i} * stats.rms * wn.d{i} as d{i}" for i in range(HIDDEN_DIM))
    sql.append(f"""
        CREATE TRIGGER IF NOT EXISTS llm_final
        AFTER INSERT ON step_30
        BEGIN
...
-- IMPORTANT FIX: FILE avoids RAM starvation
PRAGMA journal_mode = OFF;
PRAGMA synchronous = 0;
